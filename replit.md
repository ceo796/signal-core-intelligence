...
- **Document library:** upload (PDF/DOCX/TXT/CSV/XLSX/XLS), list, detail page with tabbed Preview (in-platform PDF viewer; spreadsheets render a polished client-side **grid** with workbook/sheet tabs + pagination and a secondary "View extracted text" debug toggle — extraction/indexing unchanged) / Extracted Text / Citations / History / System, download original, re-index, delete. The detail page is a **two-column workspace**: document info/tabs on the left, an embedded **Hybrid AI Agent** panel on the right ("Ask across all your documents.") that defaults to all-document scope and can be narrowed to this-document / selected-documents (reuses `POST /api/agent/hybrid`; citations + Verification Trace preserved).
...
- **Admin stats** (`/admin`).
- **Hybrid AI Chat** (`/agents/hybrid`): the prominent cross-document assistant (**"AI Chat"**, 2nd nav item), presented as a **ChatGPT-style composer** — a centered textbox with the Mode selector as a pill-button dropdown and the Documents selector as a dropdown underneath, plus a disabled "Web · Soon" placeholder pill; after asking, the composer docks to the bottom and the answer renders above. Ask one question across all (or selected) documents; answers are grounded in your documents with `[Source N]` citations and supplemented by the OpenAI GPT model's own reasoning — **no web research**. Each answer shows source labels (`document_context` / `gpt_reasoning` / disabled `web_context_placeholder_disabled`); a future web pathway is present only as a visibly disabled placeholder.
