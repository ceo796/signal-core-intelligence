...
async function fetchOriginalBlobWithRetry(documentId: number): Promise<Blob> {
  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    try {
      ...
      if (blob.size === 0) throw new Error("Original PDF response was empty.");
      return blob;
    } catch (err) {
      lastError = err;
  ...
  throw lastError instanceof Error ? lastError : new Error("Original PDF could not be retrieved.");
}
        ...
        <div className="mb-4 rounded-2xl border border-amber-200 bg-amber-50 p-3 text-amber-950">
          <div className="flex items-start gap-2.5">
            ...
            <div>
              ...
              <p className="mt-1 text-[11px] leading-relaxed text-amber-900/80">{reason} Showing indexed text so the document remains readable.</p>
            </div>
          </div>
        </div>
...
export default function DocumentDetailStable() {
  ...
  const handleOpenOriginal = async () => {
    ...
    try {
      const blob = await fetchOriginalBlobWithRetry(id);
  ...
  const renderViewer = () => {
    if (isPdf) {
      ...
      if (pdfUrl) return <div className="flex-1 min-h-0"><PdfViewer fileUrl={pdfUrl} onDownload={handleDownload} /></div>;
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden">
        <header className="shrink-0 border-b border-border bg-card/60 px-4 md:px-6 py-2.5">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <span className="hidden sm:inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-medium shrink-0" style={{ backgroundColor: "#f4e8e1", color: "#8a6f60" }}><FileText className="w-3 h-3" />{doc.fileType.toUpperCase()}</span>
          ...
          {!status.isReady && <div className="mt-2 flex items-start gap-2 rounded-md border border-amber-200 bg-amber-50 p-2.5"><AlertTriangle className="w-3.5 h-3.5 text-amber-600 shrink-0 mt-0.5" /><p className="text-[11px] text-amber-900 leading-relaxed">{status.description}</p></div>}
        </header>
